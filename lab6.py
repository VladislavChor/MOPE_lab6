...
d_x1 = (x1_max - x1_min) / 2
d_x2 = (x2_max - x2_min) / 2
d_x3 = (x2_max - x3_min) / 2
x0_1 = (x1_min + x1_max) / 2
x0_2 = (x2_min + x2_max) / 2
x0_3 = (x3_min + x3_max) / 2
...
def main(m, n):
    if n == 14:
        ...
        x = [
            ...
            [1, -const_l * d_x1 + x0_1, x0_2, x0_3],
            [1, const_l * d_x1 + x0_1, x0_2, x0_3],
            [1, x0_1, -const_l * d_x2 + x0_2, x0_3],
            [1, x0_1, const_l * d_x2 + x0_2, x0_3],
            [1, x0_1, x0_2, -const_l * d_x3 + x0_3],
            [1, x0_1, x0_2, const_l * d_x3 + x0_3],
        ]
    ...
    if n == 14:
        y = [[round(sum([values[j] * i[j] for j in range(len(values))]) + random() * 10 - 5, 2) for k in range(m)] for i
             in x]
    else:
    ...
    for i in range(len(x[0])):
        m_ij.append([round(sum([x[k][i] * x[k][j] for k in range(len(x))]) / 14, 2) for j in range(len(x[i]))])
